  ...
  describe("mutations", () => {
    ...
    it("complains about ambiguous types", async () => {
      try {
        ...
        expect(e.message).toMatchInlineSnapshot(
          `"RelayMockNetworkLayerError: Ambiguous object at path 'commerceBuyerAcceptOffer/orderOrError/order'. Add a __typename from this list: [BuyOrder, OfferOrder]"`
        )
      }
    })
    ...
    it("complains if you return the wrong type in an abstract position", async () => {
      try {
        ...
        expect(e.message).toMatchInlineSnapshot(
          `"RelayMockNetworkLayerError: Expected object of type 'Order' but got 'string' at path 'commerceBuyerAcceptOffer/orderOrError/order'"`
        )
      }
    })
  })
